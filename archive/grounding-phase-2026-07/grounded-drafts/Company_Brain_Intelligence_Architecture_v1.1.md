...
### Status
Canonical Conceptual Model — Frozen, Grounding Addendum Applied (pending human review and freeze of this addendum)

### Supersedes
Planning documents `Intelligence_Architecture_v1__reasoning_1`, `Intelligence_Architecture_v1__planning_1`, draft `Intelligence_Architecture_v1_draft`; grounds Version 1.0.
...
Per Master Plan §2.5, this document is treated as frozen: this is an addendum, not a content edit. Two inline citation callouts were added (Part 6, Part 13) where Master Plan §2.5 identifies a real academic parent, rewritten this pass into theory-first exposition (theory stated plainly, then an explicit implement/extend bridge) rather than citation-then-mention. No reasoning, lifecycle, object model, or freeze decision below was changed. Full citation detail: `Company_Brain_Theoretical_Foundations_v1.md`.

- Part 6 (Reasoning Pipeline by Consultant Mode) — grounded in Herbert Simon's Intelligence–Design–Choice model, *The New Science of Management Decision* (1960). Fit: Clean for Inquiry/Planning/Review (direct implementation); Simulation Mode is an explicit extension.
- Part 13 (Confidence Model) — noted as a forward-looking, light grounding opportunity in calibration/Brier-score vocabulary from decision theory. Per Master Plan §2.5, this is explicitly flagged for Technical Architecture rather than grounded further here, since exact scoring mechanics are already deferred there (see "Still Open," below).
- Parts 7–10 (Recommendation/Risk/Opportunity/Drift Intelligence) and Part 15 (Brain Score, Agent Readiness) remain **KEEP** — Master Plan §2.5 finds no academic equivalent and treats these as genuine original contribution. Not force-fit to any citation.
...
- Exact confidence scoring mechanics (weights, formula) — Technical Architecture. Calibration/Brier-score vocabulary flagged above as the eventual grounding source when this is picked up.
...
- Single home for trust/governance concerns spanning this and other documents — now addressed by Trust & Governance Architecture v1.0 / v1.1

This document's reasoning content is frozen. No further Intelligence Architecture content revisions are planned before Technical Architecture begins. This Grounding Addendum adds citations only.
...
Herbert Simon's *The New Science of Management Decision* (1960) breaks decision-making into three phases — **Intelligence** (gathering evidence about the situation), **Design** (generating a course of action), and **Choice** (evaluating against the intended outcome) — verified as Simon's original three-phase model, with a fourth phase (Implementation) appearing only in his later work, not this one. The Reasoning Pipeline below implements this directly at the mechanism level: Inquiry Mode's pipeline stops at Explanation the same way Simon's Intelligence phase stops at gathering evidence; Planning Mode runs the full pipeline to Recommendation Generation, matching Design; Review Mode compares actual against intended and only generates a recommendation if something actionable surfaces, matching Choice. Simulation Mode extends beyond Simon's three phases — scenario generation and outcome comparison aren't in the 1960 model, and this document doesn't force them to look like a borrowed fourth phase. See `Company_Brain_Theoretical_Foundations_v1.md`, Field 9 background and the Product Architecture v2.2 Consultant Modes grounding, which implements the same mapping one layer up.

**Relationship:** we implement Simon's three-phase model directly for Inquiry/Planning/Review mode branching; Simulation Mode is an explicit extension.
...
*No external grounding forced — Master Plan §2.5 marks Parts 7–10 KEEP: no academic equivalent found, genuine original contribution.*
...
*No external grounding forced for the Drift pipeline itself (Candidate → Pattern → Signal → Severity) — Master Plan §5 identifies this as original IP with no academic or industry equivalent. The underlying "why intended and observed reality diverge" question is grounded at the Memory Model level (Memory Model v1.3 §12, Weick and Argyris & Schön) — this document's pipeline mechanics remain KEEP.*
...
The Consultant is an intelligence consumer. See Part 6 for per-mode pipeline behavior and its grounding in Simon's Intelligence–Design–Choice model.
...
### Theoretical Grounding — Not Yet Applied

Decision theory's calibration and Brier-score vocabulary give a formal way to score how well a stated confidence level matches actual outcomes over time — the eventual academic parent for a *quantified* version of this model. Master Plan §2.5 flags this deliberately for Technical Architecture rather than this conceptual document, since exact scoring mechanics are already deferred there (see Document Control, "Still Open"). No citation is forced onto the conceptual levels below — the honest relationship here is **not yet applicable**, not "consistent with" or "implements": this document defines confidence conceptually (Very Low through Very High, with asymmetric handling by output type), and the theory that would ground a numeric version of it doesn't get retrofitted onto a model that isn't numeric yet.
...
**Write Path:** validated learning does not write directly to memory. It is proposed as a Learning Memory candidate and passes through Memory Model v1.3's standard Write Governance (Section 9) — AI-proposed writes require automatic, delegated, or human approval depending on type. Intelligence proposes; Memory governs the write. This is the same boundary Memory Model v1.3 already establishes for all AI-originated memory — Intelligence Architecture does not introduce a second path around it.

---

# Part 15 — Intelligence Object Model

*No external grounding forced for the object model itself — Master Plan §2.5 marks Part 15 (Brain Score, Agent Readiness) KEEP: original output types, preserved as differentiator.*
...
This mirrors Memory Model v1.3's distinction:
...
**Decay applies only to Intelligence Objects in active/open lifecycle states** (e.g. Detected, Validated, Tracked, Presented, Investigated, Escalated, Assessed). Once an object reaches a terminal state — Resolved, Accepted / Rejected / Modified, Pursued / Dismissed, Confirmed / Falsified, Addressed / Accepted, Certified — it stops decaying and retains its confidence as a historical record, mirroring Memory Model v1.3's Archive Path: archived records remain historically true and are not silently altered.
...
Cross-document alignment with Ontology v1.3, Section 7 (Derived Intelligence Objects).

**Approved Intelligence Outputs** *(matching Ontology v1.3 exactly — ten of ten)*:
...
- Confidence scoring formula *(eventual grounding: calibration/Brier-score vocabulary, decision theory — see Document Control)*
...
Deferred to Trust & Governance Architecture v1.1 (now addressed):
...
The Intelligence Architecture defines how the Company Brain transforms memory into contextual understanding, recommendations, risk awareness, opportunity awareness, and drift detection — through a confidence model that behaves asymmetrically by output type, a freshness mechanism that decays confidence without silently resolving or erasing intelligence, explicit ownership and review triggers on every output, and a learning loop that writes back to memory only through Memory Model v1.3's existing governance — while preserving explainability, confidence, and trust.

---

# Freeze Statement

With Ownership, Review Triggers, Intelligence Freshness, and Confidence Decay incorporated and reconciled against the rest of the document (decay scoped to active states only, review trigger scope narrowed in Part 19), and now with a Grounding Addendum applied (Part 6, Part 13 citation callouts; Parts 7–10, 15 confirmed as original IP), Intelligence Architecture v1.1 is frozen pending human review of this addendum.

No further Intelligence Architecture content revisions are planned before:

```text
Technical Architecture v1
```

Remaining open items are implementation questions for Technical Architecture (see Document Control, "Still Open").
