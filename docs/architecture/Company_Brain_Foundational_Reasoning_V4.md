...
*(Status table below updated July 2026 at the freeze of the Theoretical Grounding Phase — see §25. This is the one section of this document, besides the new §25 itself, touched after Chapter 24; Chapters 1–24 remain exactly as originally written, per this document's own append-only rule.)*

This is not a spec. It is the reasoning trail behind the specs.

Eight canonical documents currently define the Company Brain. They are the
...
- the CTO has a single place to reconstruct project context without
  re-reading eight formal documents end to end, and
...
### Canonical Documents (status as of this writing — post-freeze, Theoretical Grounding Phase)

| Document | Version | Status | Defines |
|---|---|---|---|
| Architecture & Vision | v2.3 | Canonical | The seven-layer pipeline (incl. Exposure), atomic primitives, Primitive Knowledge Objects, memory architecture overview, commitment lifecycle, designed/executed drift, provenance requirements — grounded in Beer's VSM, Winograd & Flores, Zachman, and others; see Theoretical Foundations |
| Ontology | v1.3 | Canonical | What first-class objects exist (Core Objects, Organizational Structures, Operational Constructs), their composition, Composite Knowledge Objects, their memory homes, their relationships — grounded in Dietz/DEMO |
| Memory Model | v1.3 | Canonical | How reality becomes memory: Knowledge Representation, formation routing, the five memory types in full, lifecycle, decay, provenance, write governance, conflict resolution, per-subtype commitment lifecycle, drift, learning loop, Memory Consumption & Delivery — grounded in Walsh & Ungson, Argyris & Schön |
| Product Architecture | v2.2 | Canonical | How organizational memory reaches humans and AI systems: three exposure modes, Knowledge Exchange, OKF compatibility (adopted, real spec), Brain-to-Brain interoperability, governance, MVP prioritization |
| Intelligence Architecture | v1.1 | Canonical | How memory becomes understanding: context assembly, reasoning pipeline, recommendation/risk/opportunity/drift intelligence, Consultant reasoning, Agent intelligence, confidence model, adaptive intelligence — grounded in Simon's Intelligence-Design-Choice model |
| Trust & Governance Architecture | v1.1 | Canonical | Authority, accountability, provenance, challenge, approval, delegation, revocation, and oversight mechanisms across humans, agents, and automation — grounded in W3C PROV-O, RBAC/ABAC |
| Theoretical Foundations | v1.0 | Canonical | The 13 academic fields and industry standards underlying the architecture, each stated theory-first then bridged explicitly to what Company Brain implements, extends, or diverges from; the VSM Compression Test; the vocabulary rename table |
| Bibliography | — | Canonical | Full verified reference list for every citation used across the document set |
| Foundational Reasoning | v4.0 (this file) | Reference only | Why each of the above exists, in what order, and what each one fixed in the one before it, including the Theoretical Grounding Phase itself (§25) |

**Not yet started:** Technical Architecture, MVP Architecture.
...
3. **Open Knowledge Format (OKF).** A representation-layer question that
...
but answering it honestly required two passes (preserved in
`OKF_Reasoning_1` and `OKF_Reasoning_2`), because the first answer was too
shallow.
...
## 25. The Theoretical Grounding Phase — Vocabulary Without New Architecture

*(Appended July 2026. Chapters 1–24 above are unedited — per the Master
Plan's explicit rule for this document, grounding is logged here, not
retrofitted into the narrative chapters. Chapter 0's status table is the
one exception: at freeze of this phase, the team explicitly authorized
updating it to reflect the eight canonical documents and their final
versions (see the table above) — a status table is meant to be current,
unlike the chapters, which are meant to preserve what was true at the time
they were written. Chapters 22–23's own narrative and open-questions tables
still describe Intelligence Architecture and Trust & Governance as
not-yet-started as of when they were written; that historical staleness is
left as-is, consistent with the append-only rule for narrative content —
it is a byproduct of exactly the kind of drift Chapter 15's heuristic
exists to catch, just not in scope to fix in the narrative itself.)*

By the time Trust & Governance Architecture v1.0 froze, the project had six
canonical documents and a recurring team complaint that predates any of the
content fixes in Chapters 11–20: the vocabulary was hard for new technical
hires to parse, because almost all of it was invented fresh rather than
drawn from any recognizable prior art. A second, related concern sat behind
the first: a concept with thirty to fifty years of field-testing (Beer's
Viable System Model, Dietz's DEMO, Walsh & Ungson's organizational memory
theory) is lower-risk to build against than a fully bespoke one, and nobody
had actually checked how much of the architecture already had that kind of
parent theory sitting under it, unrecognized.

The Theoretical Grounding & Vocabulary Alignment Master Plan (dated July 1,
2026) scoped a answer to both concerns as a single, tightly-bounded phase:
mine thirteen academic fields and industry standards, map each onto the
existing architecture section by section, cite what fits, and — critically
— explicitly flag what doesn't, rather than force a citation onto Company
Brain's actual original contributions. The plan's own framing, verbatim: "Not
new concepts. Not a rewrite." Every citation was independently verified by
web search before use, including the two time-sensitive ones least likely
to still be accurate by the time anyone read this — OKF v0.1 and MCP's
governance handoff — both of which held up.

**What got mapped to what.** The strongest, cleanest fits landed on the
documents already closest to established theory in spirit, not surprisingly
the same two the project had independently converged toward field-tested
territory on back in Chapter 10 (the Atomic Primitive Discovery): Ontology
mapped almost completely onto Jan Dietz's DEMO/Enterprise Ontology — the
same "abstracted from implementation" philosophy, the same coordination/
production-act split underlying Commitment. Memory Model's Drift/Learning
pipeline mapped tightly onto Argyris & Schön's single-loop/double-loop
learning distinction — a single Drift Candidate correction is single-loop,
a confirmed Drift Pattern routed into Learning Memory as a policy-
improvement candidate is double-loop, close to verbatim. Trust & Governance's
Provenance Architecture mapped 1:1 onto W3C's PROV-O vocabulary, the single
strongest rename opportunity found anywhere in the document set. The full
field-by-field mapping, with citations, summaries, and fit ratings, lives in
the new `Company_Brain_Theoretical_Foundations_v1.md` — not repeated here.

**The VSM Compression Test — the one exercise the Master Plan required
before any rename could be called final.** Beer's five-subsystem Viable
System Model was tested as a parent theory for the seven-layer pipeline by
attempting to re-derive the seven layers from the five subsystems. Five of
seven compressed cleanly: Capture+Understand as the sensing function
(System 1/2), Execution as System 1, Intelligence as System 4, Trust &
Governance as System 5 (outside the pipeline proper, but a clean fit), and
Evolution as a partial match to the System 3–4 feedback loop. Two did not:
Memory and Exposure, as first-class named layers, have no VSM equivalent —
VSM assumes both stay implicit inside an organization's informal channels.
This produced the one required sentence the Master Plan asked for, now
carried in Theoretical Foundations verbatim: Company Brain adopts VSM's
viability logic and extends it with the two layers VSM left implicit,
because in an AI-native organization memory and delivery can no longer stay
implicit. This is the same shape of finding as Chapter 10's primitive
convergence — independent validation of what's borrowed and a precise
boundary around what's genuinely new — just run in the opposite direction
(testing an adopted theory for its limits, rather than discovering
primitives that happened to match one).

**OKF — adopted, not merely cited.** Unlike every other citation in this
phase, Open Knowledge Format was not just grounded, it was resolved. The
project's own internal placeholder term "Open Knowledge Format (OKF),"
present since Chapter 18, is retired in favor of Google Cloud's real,
published OKF v0.1 (verified live: June 12, 2026, Apache 2.0, markdown +
YAML frontmatter, shipping reference tooling). This is a stronger move than
the OKF reasoning in Chapter 18 originally committed to — Chapter 18 treated
OKF as a compatibility target with unspecified content; this phase gives it
a concrete field-level mapping (Composite Knowledge Object → OKF concept
document, `type` field required, relationships as markdown links) in a new
standalone deliverable, `OKF_Adoption_Mapping.md`. The division of labor
Chapter 18 anticipated — OKF solves portability, Company Brain solves
governance — held up exactly as predicted; nothing about Trust, Provenance,
Conflict Resolution, or Decay needed to change to accommodate the real
spec, because OKF v0.1 explicitly leaves all four as open design space.

**What stayed exactly as it was — the actual find.** Per the Master Plan's
own framing, the more useful output of this phase is arguably the KEEP
list, not the GROUND list: Drift Detection's Candidate→Pattern→Signal→
Severity pipeline, Commitment Memory and Learning Memory as persistent,
stateful, organization-level memory types with no individual-cognition or
single-conversation equivalent, Brain Score and Agent Readiness as
Intelligence Architecture output types, Right Memory/Right Moment and the
Exposure Layer as the VSM-extension finding above already demonstrates, and
Knowledge Exchange/Brain-to-Brain Interoperability as a strategic direction
rather than a format-compatibility claim. None of these were forced onto a
citation that didn't fit. This list is now the direct answer, cited and
specific, to "what's actually new here" — a question this project has
implicitly been answering since Chapter 5's "Company Brain ≠ Operational
Intelligence" distinction, now made explicit with citations attached to
everything around it.

**What this phase did not do.** No layer, primitive, memory type, object,
or lifecycle changed, at draft time or at freeze. The six architecture
documents were version-bumped (Architecture & Vision v2.2→v2.3, Ontology
v1.2→v1.3, Memory Model v1.2→v1.3, Product Architecture v2.1→v2.2,
Intelligence Architecture v1.0→v1.1, Trust & Governance v1.0→v1.1) and, after
a team review pass that required a second round of exposition rewriting
(theory stated first, then bridged explicitly to what Company Brain
implements, extends, or diverges from — not citation-then-mention), the team
approved the result and this phase froze: the six grounded drafts replaced
the prior canonical versions in place, Theoretical Foundations and
Bibliography joined the canonical set as new documents, and the full
pre-freeze working history — every intermediate draft, the original
pre-grounding document versions, and the team-review artifacts (change log,
coverage check, consistency check, recognizability sheet) — was preserved in
`/archive/grounding-phase-2026-07/` rather than discarded. This document
itself was appended to for this section, plus one narrow, explicitly
authorized correction to Chapter 0's status table to keep it current;
Chapters 1–24 remain exactly as originally written, per this document's own
founding rule: if this file conflicts with a canonical document, the
canonical document wins, and this file is the map, not the territory.

Roadmap position: Technical Architecture is next, and it now has field-tested
prior art — DEMO, VSM, PROV-O, Walsh & Ungson, Argyris & Schön, a real
external interchange format in OKF — to build against, instead of only this
project's own internal documents.
