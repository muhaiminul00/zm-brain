...
Canonical architecture documentation for "Company Brain" (ZeroManual, internal). The eight documents below are the frozen conceptual grounding phase — see `docs/designs/` for the active Technical Architecture / Building phase, which started 2026-09-02 and has its v1 scope and architecture locked (see `.project-memory/PROJECT_STATE.md` for current status).
...
- `marked_for_technical_architecture.md` — **treat as dead; `TODOS.md` (repo root) is the actual parking lot going forward.** Still doesn't exist (verified again 2026-09-02, and again by an adversarial-review pass at `/ship` the same day, which flagged this line as a documentation trap — future contributors could look for a non-existent file instead of `TODOS.md`). Technical Architecture starting was this line's own stated trigger to create it; instead, `/plan-eng-review`'s deferred items went to `TODOS.md` and its unresolved items to `docs/designs/zm-brain-v1-context-sync.md`'s closing sections. Do not create this file — if something doesn't fit either of those two homes, that's a signal to reconsider where it belongs, not to revive this line.
...
Per Foundational Reasoning V4's roadmap, Technical Architecture was next after
this grounding phase — **it has now started** (2026-09-02): a `/office-hours`
session narrowed v1 scope to a D2C wedge, and `/plan-eng-review` has locked
the v1 architecture — meaning the *decisions* are made (store, retrieval,
sync, delivery), not that the remaining risk is resolved: Eng Review status
is explicitly **ISSUES_OPEN / NOT CLEAR** (unresolved decisions + critical
failure-mode gaps, tracked in the design doc, not silently dropped). See
`docs/designs/zm-brain-v1-context-sync.md` for the
design doc and its "Technical Architecture Lock-In" / "GSTACK REVIEW REPORT"
sections, and `.project-memory/PROJECT_STATE.md` for current status. The
grounded document set above remains this work's field-tested prior art (VSM,
DEMO, Organizational Memory Theory, PROV-O, and others — see
`Company_Brain_Theoretical_Foundations_v1.md`) — a reference starting point
to re-verify against, per `CLAUDE.md`'s standing rule, not a ready-to-build
spec.
